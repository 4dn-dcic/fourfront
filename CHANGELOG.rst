...
* Add a comment to ``base.ini`` to make it align properly with cgap-portal.
  (There are still other important differences. This just wasn't important.)

* In ``dev_servers.py``, syntactic leveling with CGAP's version of this file,
  including some disabled functionality that is presently CGAP-only but soon
  will not be.

* In ``server_defaults.py``, syntactic leveling with CGAP's version of this file.
