...
* File upload/download credentials: replace ``sts:GetFederationToken`` with ``sts:AssumeRole`` in ``external_creds()`` (``types/file.py``), matching the already-landed ``encoded-core`` 1.0.0 and ``cgap-portal`` migrations. ``GetFederationToken`` is only accepted from an IAM user holding long-lived access keys and is rejected outright when the caller itself holds temporary credentials, so it fails under every role-based caller -- ECS task roles and, since 8.9.2, the GitHub Actions OIDC role. The role is read from ``S3_UPLOAD_ROLE_ARN`` (identity first, environment as fallback) and no access keys are passed to ``boto3``. The session policy is unchanged, so the credentials remain scoped to ``s3:PutObject`` on the single target key; response mapping moves from ``FederatedUser`` to ``AssumedRoleUser``, and the public ``upload_credentials``/``upload_url`` keys are unchanged. ``upload_key`` now also catches ``BotoCoreError`` so a missing role ARN keeps degrading to a message instead of a 500.
