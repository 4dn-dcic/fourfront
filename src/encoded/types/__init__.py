...
class SopMap(Item):
    ...
    def mapid(self, request, schema_version, associated_item_type, id_values=None):
        delim = '_'
        id_string = associated_item_type + delim
        if id_values is not None:
            for f in id_values:
                f = ''.join(f.split())
                id_string = id_string + f + delim
        return id_string + schema_version
