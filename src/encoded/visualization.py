...
#UNCOMMENT ME @view_config(route_name='add_files_to_higlass_viewconf', request_method='POST')
def add_files_to_higlass_viewconf2(request):
...
def add_single_file_to_higlass_viewconf2(views, new_file, genome_assembly):
...
def repack_higlass_views2(views):
...
def add_zoom_lock_if_needed2(view_config, view, scales_and_center_k):
...
def remove_left_side_if_all_1D2(new_views):
...
def copy_top_reference_tracks_into_left2(target_view, views):
...
def add_2d_file(views, new_content, viewconfig_info):
    ...
    if not base_view_info["has_center_content"]:
        ...
        if len(views[0]["tracks"]["center"]) == 0:
            views[0]["tracks"]["center"] = [{"contents":[]}]
