...
class FourfrontRoot(Root):
    ...
    def is_mobile(self, request):
        if hasattr(request, 'user_agent'):
            ...
            b = _IS_MOBILE_MATCH_B.search(user_agent)
            v = _IS_MOBILE_MATCH_V.search(user_agent[0:4])
            if b or v:
                return True      
        # fallback
        return False