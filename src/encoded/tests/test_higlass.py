...
@pytest.fixture
def higlass_mcool_viewconf(testapp, award, lab):
...
@pytest.fixture
def higlass_blank_viewconf(testapp, lab, award):
    ...
    viewconf = {
        ...
        "viewconfig": {
            ...
            "zoomFixed":
            False,
            "trackSourceServers": ["//higlass.4dnucleome.org/api/v1"],
            "exportViewUrl":
            "/api/v1/viewconfs",
            "views": [{
                "uid":
                "aa",
                "initialXDomain": [-167962308.59835115, 3260659599.528857],
                "chromInfoPath":
...
def test_higlass_noop(testapp, higlass_mcool_viewconf):
    ...
    response = testapp.post_json("/add_files_to_higlass_viewconf/", {
        'higlass_viewconfig':
        higlass_json["viewconfig"],
        'genome_assembly':
        higlass_json["genome_assembly"],
        'files': [],
    })
    ...
    assert_true(
        len(higlass_json["viewconfig"]["views"]) == len(
            new_higlass_json["views"]))
...
def test_add_mcool(testapp, higlass_blank_viewconf, mcool_file_json):
    ...
    assert_true(
        ...
        "initialXDomain left side is wrong. Should be around -681380342, instead got "
        + str(view["initialXDomain"][0]))
    assert_true(
        abs(view["initialXDomain"][1] - 3406901712) < 100,
        "initialXDomain right side is wrong. Should be around 3406901712, instead got "
        + str(view["initialXDomain"][1]))
    assert_true(
        abs(view["initialYDomain"][0] + 681380342) < 100,
        "initialYDomain top side is wrong. Should be around -681380342, instead got "
        + str(view["initialYDomain"][0]))
    assert_true(
        abs(view["initialYDomain"][1] - 3406901712) < 100,
        "initialYDomain bottom side is wrong. Should be around 3406901712, instead got "
        + str(view["initialYDomain"][1]))


def test_add_bedGraph_higlass(testapp, higlass_mcool_viewconf,
                              bedGraph_file_json):
    ...
    bg_file = testapp.post_json('/file_processed',
                                bedGraph_file_json).json['@graph'][0]
    ...
    bedGraph_track = [
        t for t in tracks["top"] if "-divergent-bar" in t["type"]
    ][0]
    assert_true(
        bedGraph_track["height"] < 100,
        "1D file is too big: height should be less than 100, got {actual} instead.".
        format(
            actual=bedGraph_track["height"], ))


def test_add_bedGraph_to_bedGraph(testapp, higlass_blank_viewconf,
                                  bedGraph_file_json):
    """ Given a viewconf with an mcool file, the viewconf should add a bedGraph on top.
    ...
    assert_true(
        isinstance(new_higlass_json["views"][0]["tracks"]["top"][0]["height"],
                   int), "Not an integer")

    # The new top track should be tall, since there are no other tracks. But not too tall.
    assert_true(
        new_higlass_json["views"][0]["tracks"]["top"][0]["height"] >= 100
        and new_higlass_json["views"][0]["tracks"]["top"][0]["height"] < 600,
        "1D file is wrong size: height should be at least 100 and less than 600, got {actual} instead.".
        format(
            actual=new_higlass_json["views"][0]["tracks"]["top"][0]["height"],
        ))

    # Make sure there is a label.
    assert_true(new_higlass_json["views"][0]["tracks"]["top"][0]["options"]
                ["labelPosition"] != "hidden")
    ...
    bedGraph_tracks = [
        t for t in new_higlass_json["views"][0]["tracks"]["top"]
        if "-divergent-bar" in t["type"]
    ]
    for t in bedGraph_tracks:
        assert_true(
            t["height"] >= 100,
            "1D file is too small: height should be at least 100, got {actual} instead.".
            format(
                actual=t["height"], ))
...
def test_add_mcool_to_mcool(testapp, higlass_mcool_viewconf, mcool_file_json):
    ...
    mcool_file_with_different_genome_assembly = testapp.post_json(
        '/file_processed', mcool_file_json).json['@graph'][0]
    ...
    response = testapp.post_json(
        "/add_files_to_higlass_viewconf/", {
            ...
            'files': [
                "{uuid}".format(
                    uuid=mcool_file_with_different_genome_assembly['uuid'])
            ]
        })

    assert_true(response.json["success"] == False)
    assert_true("All files are not" in response.json["errors"])
    assert_true(mcool_file_with_different_genome_assembly['uuid'] in
    ...
    for lock_name in ("locationLocks", "zoomLocks"):
        ...
        assert_true(locks["locksByViewUid"][view0_uid] == locks[
            "locksByViewUid"][view1_uid])

        lockUuid = locks["locksByViewUid"][view0_uid]
        # The locks have non-None values
        for view_uid in (view0_uid, view1_uid):
            for index, lock_value in enumerate(
                    locks["locksDict"][lockUuid][view_uid]):
                assert_true(
                    lock_value != None,
                    "{lock_name} for view  {view_uid} should not be None: {actual}".
                    format(
                        lock_name=lock_name,
                        view_uid=view_uid,
        ...
        assert_true(
            len(locks["locksDict"][lockUuid][view0_uid]) == len(
                locks["locksDict"][lockUuid][view1_uid]))

        for index, lock_value in enumerate(
                locks["locksDict"][lockUuid][view0_uid]):
            assert_true(
                lock_value == locks["locksDict"][lockUuid][view0_uid][index],
                "{lock_name} values do not match for index {index}. Expected {lock_value}, Actual {actual}".
                format(
                    lock_name=lock_name,
...
def test_correct_duplicate_tracks(testapp, higlass_mcool_viewconf,
                                  mcool_file_json):
    ...
    for side in ("top", "left"):
        assert_true(
            len(view0["tracks"][side]) == 2,
            "{side} does not have 2 tracks".format(side=side))
        ...
        for i in range(len(view0["tracks"][side])):
            assert_true(
                view0["tracks"][side][i]["uid"] == view1["tracks"][side][i][
                    "uid"],
                "{side} track {index} do not match: {zero} versus {one}".
                format(
                    side=side,
                    index=i,
...
def assert_expected_viewconf_dimensions(viewconf, expected_dimensions):
    ...
    assert_true(
        ...
        "Expected {num_expected} views, but there were {num_actual} views instead.".
        format(
    ...
    for index, expected_layout in enumerate(expected_dimensions):
        ...
        for dimension in ("x", "y", "w", "h"):
            assert_true(
                layout[dimension] == expected_layout[dimension],
                "While looking at {num_expected} expected dimensions, index: {index}, dimension: {dim} mismatched.".
                format(
                    num_expected=len(expected_dimensions),
                    index=index,
                    dim=dimension))
...
def test_add_multiple_mcool_one_at_a_time(testapp, higlass_mcool_viewconf,
                                          mcool_file_json):
    ...
    assert_true(response.json["success"] == False)
    assert_true("You cannot have more than 6 views in a single display." in
                response.json["errors"])


def test_add_multiple_mcool_at_once(testapp, higlass_mcool_viewconf,
                                    mcool_file_json):
    """ Make sure you can add multiple mcool displays together, up to six.
    Eventually we'll see a 3 x 2 grid.
    ...
    response = testapp.post_json("/add_files_to_higlass_viewconf/", {
        ...
        higlass_json["genome_assembly"],
        'files': [
            mcool_file['uuid'], mcool_file['uuid'], mcool_file['uuid'],
            mcool_file['uuid']
        ]
    })
    assert_true(response.json["success"] == False)
...
def test_add_bedGraph_to_multiple_mcool(
        testapp, mcool_file_json, higlass_mcool_viewconf, bedGraph_file_json):
...
def test_add_new_mcool_file(testapp, mcool_file_json, higlass_mcool_viewconf,
                            bedGraph_file_json):
    ...
    old_top_track_count = len(
        higlass_json["viewconfig"]["views"][0]["tracks"]["top"])
    ...
    assert_true(
        len(new_higlass_json["views"][0]["tracks"]["top"]) ==
        old_top_track_count + 1, "913")
    assert_true(
        len(new_higlass_json["views"][0]["tracks"]["center"][0]["contents"]) ==
        1, "914")
    ...
    for index in range(len(top_track_count.keys())):
        assert_true(
            top_track_count[index] == 3,
            "Expected 3 tracks on top for view {view}, found {actual} instead.".
            format(view=index, actual=top_track_count[index]))
...
def test_add_files_by_accession(testapp, mcool_file_json,
                                higlass_blank_viewconf, bedGraph_file_json):
...
def test_add_bedGraph_to_mcool(testapp, higlass_mcool_viewconf,
                               bedGraph_file_json):
    """ Given a viewconf with a mcool file, the viewconf should add anohter mcool on the side.
    ...
    bedGraph_file_json['md5sum'] = '00000000000000000000000000000001'
    bg_file_with_different_genome_assembly = testapp.post_json(
        '/file_processed', bedGraph_file_json).json['@graph'][0]
    ...
    response = testapp.post_json(
        "/add_files_to_higlass_viewconf/", {
            ...
            'files': [
                "{uuid}".format(
                    uuid=bg_file_with_different_genome_assembly['uuid'])
            ]
        })

    assert_true(response.json["success"] == False)
    assert_true("genome assembly" in response.json["errors"])
    assert_true(bg_file_with_different_genome_assembly['uuid'] in
                response.json["errors"])
...
def test_add_bigwig_higlass(testapp, higlass_mcool_viewconf, bigwig_file_json):
    ...
    bigwig_file = testapp.post_json('/file_processed',
                                    bigwig_file_json).json['@graph'][0]
...
def test_add_bigbed_higlass(testapp, higlass_mcool_viewconf, bigbed_file_json):
    ...
    bigbed_file = testapp.post_json('/file_processed',
                                    bigbed_file_json).json['@graph'][0]
...
def test_add_bed_with_beddb(testapp, higlass_mcool_viewconf,
                            bed_beddb_file_json):
    ...
    bed_file = testapp.post_json('/file_processed',
                                 bed_beddb_file_json).json['@graph'][0]
...
def test_add_beddb(testapp, higlass_mcool_viewconf, beddb_file_json):
    ...
    assert_true(
        main_view["genomePositionSearchBox"]["chromInfoId"] == genome_assembly)
    assert_true("autocompleteId" in main_view["genomePositionSearchBox"])
    assert_true(main_view["genomePositionSearchBox"]["autocompleteId"] ==
    ...
    assert_true(
        beddb_file_json['higlass_uid'] in main_view["autocompleteSource"])
...
def test_add_chromsizes(testapp, higlass_blank_viewconf, chromsizes_file_json):
    ...
    chrom_file = testapp.post_json('/file_reference',
                                   chromsizes_file_json).json['@graph'][0]
...
def test_add_2d_chromsizes(testapp, higlass_blank_viewconf,
                           chromsizes_file_json, mcool_file_json):
    """ Add a chromsizes file and add a top, left and center tracks to the view.
    ...
    for side in ("top", "left"):
        ...
        assert_true(tracks[side][-1]["tilesetUid"] == chromsizes_file_json[
            'higlass_uid'])
...
def test_remove_1d(testapp, higlass_mcool_viewconf, chromsizes_file_json,
                   bigwig_file_json, mcool_file_json):
    genome_assembly = "GRCm38"
    ...
    assert_true(
        len(higlass_json["viewconfig"]["views"][0]["tracks"]["left"]) +
        1 == len(full_higlass_json["views"][0]["tracks"]["left"]),
        "left side mismatch")

    assert_true(
        len(higlass_json["viewconfig"]["views"][0]["tracks"]["top"]) +
        1 == len(full_higlass_json["views"][0]["tracks"]["top"]),
        "top side mismatch")

    assert_true(full_higlass_json["views"][0]["tracks"]["top"][0]["type"],
    ...
    assert_true(
        len(all_1d_higlass_json["views"][0]["tracks"]["left"]) == 0,
        "Left tracks found")

    top_chromsizes_tracks = [
        t for t in all_1d_higlass_json["views"][0]["tracks"]["top"]
        if t["type"] == "horizontal-chromosome-labels"
    ]
    assert_true(top_chromsizes_tracks[0]["height"] == 50,
                "Top chromsize track lost its height")
    ...
    found_left_chromsizes = any([
        t for t in restored_2d_track_higlass_json["views"][0]["tracks"]["left"]
        if t["type"] == "vertical-chromosome-labels"
    ])
    ...
    for track in restored_2d_track_higlass_json["views"][0]["tracks"]["left"]:
        if "uid" in track:
            assert_true(
                ...
                "Top track uid reused for left track: {uid}".format(
                    uid=track["uid"]))
        if track["type"] in types_to_find:
            types_to_find[track["type"]] += 1
    ...
    top_chromsizes_tracks = [
        t for t in restored_2d_track_higlass_json["views"][0]["tracks"]["top"]
        if t["type"] == "horizontal-chromosome-labels"
    ]
    ...
    left_chromsizes_tracks = [
        t for t in restored_2d_track_higlass_json["views"][0]["tracks"]["left"]
        if t["type"] == "vertical-chromosome-labels"
    ]
    assert_true(left_chromsizes_tracks[0]["width"] == 50,
                "Left chromsize track has no width")
    assert_true("height" not in left_chromsizes_tracks[0],
                "Left chromsize track still has a height")


def test_2d_chromsize_always_last_track(testapp, higlass_blank_viewconf,
                                        mcool_file_json, chromsizes_file_json):
    ...
    assert_true(
        len(mcool_first_viewconf["views"][0]['tracks']['center'][0][
            "contents"]) == 2)
    assert_true(mcool_first_viewconf["views"][0]['tracks']['center'][0][
        "contents"][0]["type"] == "heatmap")
    assert_true(mcool_first_viewconf["views"][0]['tracks']['center'][0][
        "contents"][1]["type"] == "2d-chromosome-grid")
    ...
    assert_true(
        len(mcool_two_viewconf["views"][1]['tracks']['center'][0]["contents"])
        == 2)
    assert_true(mcool_two_viewconf["views"][1]['tracks']['center'][0][
        "contents"][0]["type"] == "heatmap")
    assert_true(mcool_two_viewconf["views"][1]['tracks']['center'][0][
        "contents"][1]["type"] == "2d-chromosome-grid")


def test_custom_display_height(testapp, higlass_blank_viewconf,
                               bedGraph_file_json):
    ...
    assert_true((
        bedGraph_track["height"] - 125 > -5
        and bedGraph_track["height"] - 125 < 5
    ), "1D file is too big: height should be less than 125, got {actual} instead.".
                format(
                    actual=bedGraph_track["height"], ))
    ...
    for track in [t for t in tracks["top"] if "-divergent-bar" in t["type"]]:
        ...
        ), "1D file is too big: height should be around 83, got {actual} instead.".
                    format(
                        actual=track["height"], ))
