...
def test_barplot_aggregation_endpoint(workbook, es_testapp):
    ...
    # We should get back same count as from search results here. But on Travis oftentime we don't,
    # so we compare either against count of inserts --or-- count returned from regular results.
    ...
    assert ((res['total']['experiment_sets'] == count_exp_set_test_inserts)
            or (res['total']['experiment_sets'] == search_result_count))
    ...
    # We only have 4DN as single award.project in test inserts so should have values in all buckets,
    # though probably less than total.
    #
