...
def test_metadata_tsv_view(workbook, html_es_testapp):
    ...
    result_rows = [row.rstrip(' \r').split('\t')
                   for row in res.body.decode('utf-8').split('\n')]
    ...
    res2 = html_es_testapp.post('/metadata/?type=ExperimentSetReplicate',
                                {k: json.dumps(v)
                                 for k, v in res2_post_data.items()})  # NEWER URL FORMAT
...
def test_index_data_workbook(es_app, workbook, es_testapp, indexer_testapp, html_es_testapp):
    ...
    testapp_counts = es_testapp.get('/counts')
