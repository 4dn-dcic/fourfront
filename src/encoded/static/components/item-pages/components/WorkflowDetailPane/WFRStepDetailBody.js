...
const WorkflowDetailsForWorkflowNodeRow = React.memo(function WorkflowDetailsForWorkflowNodeRow({ workflow, workflow_run }){
    ...
    if (workflow) {
        ...
        innerContent = <a href={link} className="link-offset-2 link-underline link-underline-opacity-0 link-underline-opacity-100-hover">{ title }</a>;
    } else {
...
const SoftwareDetailsForWorkflowNodeRow = React.memo(function SoftwareDetailsForWorkflowNodeRow({ software: softwareList = [] }){
    ...
    if (Array.isArray(softwareList) && softwareList.length > 0){
        softwareElements = _.map(softwareList, function(sw){
            ...
            return <a href={atId} className="link-offset-2 link-underline link-underline-opacity-0 link-underline-opacity-100-hover" key={atId}>{ sw.display_title }</a>;
        });
    } else {
    ...
    stepTitleBox(){
        ...
        return (
            <div className="col-sm-6 box">
                ...
                { stepHref ?
                    <h3 className="text-500 text-truncate" data-tip={tipIfLongString(titleString)}><a className="link-offset-2 link-underline link-underline-opacity-0 link-underline-opacity-100-hover" href={stepHref}>{ titleString }</a></h3>
                    :
