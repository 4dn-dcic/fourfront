...
describe("Server rendering", function () {
    ...
    it.skip("renders the application to html", function () {
        expect(document.title).toBe(home.portal_title);
    });

    it.skip("react render http-equiv correctly", function () {
        const meta_http_equiv = document.querySelectorAll('meta[http-equiv]');
        expect(meta_http_equiv.length).not.toBe(0);
    });

    it.skip("mounts the application over the rendered html", function () {
        const props = store.getState();
        let app;
        act(()=>{
