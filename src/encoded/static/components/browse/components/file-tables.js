...
export function renderFileTitleColumn(file, field, detailIndex, fileEntryBlockProps){
    ...
    const className = 'link-underline link-underline-opacity-0 link-underline-opacity-100-hover title-of-file' + (file.accession ? ' font-monospace' : '');
...
export class RawFilesStackedTable extends React.PureComponent {
    ...
    renderExperimentBlock(exp,i){
        ...
        const showMoreExtTitle = preventExpand && experimentSet ? (
            <a href={object.itemUtil.atId(experimentSet)} className="link-underline link-underline-opacity-0 link-underline-opacity-100-hover">(view in Experiment Set)</a>
        ) : null;
        ...
        return (
            <StackedBlock key={ experimentAtId || exp.tec_rep_no || i } hideNameOnHover={false} columnClass="experiment" stripe={this.cache.oddExpRow}
                ...
                <StackedBlockName relativePosition={expFxn.fileCountFromSingleExperiment(exp) > 6}>
                    { experimentAtId ? <a href={experimentAtId} className="link-underline link-underline-opacity-0 link-underline-opacity-100-hover name-title">{ linkTitle }</a> : <span className="name-title">{ linkTitle }</span> }
                </StackedBlockName>
        ...
        return (
            <StackedBlock columnClass="biosample" hideNameOnHover={false}
                ...
                <StackedBlockName relativePosition={expsWithBiosample.length > 3 || expFxn.fileCountFromExperiments(expsWithBiosample) > 6}>
                    { biosampleAtId ?
                        <a href={biosampleAtId} className="link-underline link-underline-opacity-0 link-underline-opacity-100-hover name-title">{ biosampleTitle }</a>
        ...
        return filesGroupedByExperimentOrGlobal.map(([experimentAccession, filesForExperiment]) => {
            ...
            const nameBlock = (
                <StackedBlockName className={replicateNumbersExists ? "double-line" : ""}>
                    ...
                    { experimentAtId ? <a href={experimentAtId} className="link-underline link-underline-opacity-0 link-underline-opacity-100-hover name-title">{ nameTitle }</a> : <div className="name-title">{ nameTitle }</div> }
                </StackedBlockName>
            );
            ...
            if (preventExpand && expSetAccession) {
                ...
                showMoreExtTitle = <a href={expSetHref} className="link-underline link-underline-opacity-0 link-underline-opacity-100-hover">(view in Experiment Set)</a>;
            }
        ...
        if (val === 'PASS'){
            return (
                <span>
                    ...
                    &nbsp; { linkToReport ? <a href={linkToReport} className="link-underline link-underline-opacity-0 link-underline-opacity-100-hover" target="_blank" rel="noreferrer noopener">Pass</a> : "Pass"}
                </span>
            );
        } else if (val === 'FAIL'){
            return (
                <span>
                    ...
                    &nbsp; { linkToReport ? <a href={linkToReport} className="link-underline link-underline-opacity-0 link-underline-opacity-100-hover" target="_blank" rel="noreferrer noopener">Fail</a> : "Fail"}
                </span>
            );
        } else {
            return '-';
    ...
    static renderForFileColValue(file) {
        ...
        return (
            <React.Fragment>
                ...
                <a className="link-underline link-underline-opacity-0 link-underline-opacity-100-hover name-title" href={fileAtId}>
                    {fileTitleString}
                </a>
            </React.Fragment>
        );
