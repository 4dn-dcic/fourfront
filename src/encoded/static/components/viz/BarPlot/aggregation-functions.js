...
var aggregationFxn = module.exports = {
    ...
    genChartData : function(
        ...
        fields = _.filter(fields, function(f){
            ...
            return _.extend({}, f, {
                'terms' : {},
                'total' : aggregationFxn.createZeroCountTermObj()
            });
        });

        aggregationFxn.aggregateByType(fields, experiments, aggregate);

        if (fields.length === 1) return fields;
        return aggregationFxn.partitionFields(fields, experiments, aggregate, useOnlyPopulatedFields);
    },
    ...
    countTermForFieldFromExperimentByAggregateType : function(fieldObj, experiment, term, aggregate){
        if (!aggregate || aggregate === 'experiments'){
            aggregationFxn.countFieldTerm(fieldObj, term);
            //experiments.forEach(aggregationFxn.countFieldsTermsForExperiment.bind(this, fields));
        } else if ( aggregate === 'experiment_sets' ){
            throw new Error("Not yet built.");

        } else if ( aggregate === 'files' ){
            aggregationFxn.countFieldTerm(fieldObj, term, true, expFxn.fileCount(experiment));
        }
    },
    ...
    aggregateByType : function(fields, experiments, aggregate){
        ...
        experiments.forEach(function(exp){
            aggregationFxn.getTermsForFieldsFromExperiment(fields,exp).forEach(function(fieldTermPair, i){
                if (fields[i].field !== fieldTermPair[0]) throw new Error("This shouldn't happen");
                aggregationFxn.countFieldTerm(fields[i], fieldTermPair[1], true, 'experiments');
            });
        });
        ...
        _.forEach(
            ...
            function(expsInSet){
                ...
                fields.forEach(function(currField){
                    ...
                    aggregationFxn.getUniqueMatchedTermsFromExpsInSet(expsInSet, currField.field).forEach(function(term){
                        aggregationFxn.countFieldTerm(currField, term, true, 'experiment_sets', 1);
                    });
                });
                ...
                expsInSet.forEach(function(exp){
                    var fieldTermPairs = aggregationFxn.getTermsForFieldsFromExperiment(fields,exp);
                    fieldTermPairs.forEach(function(fieldTermPair, i){
                        if (fields[i].field !== fieldTermPair[0]) throw new Error("This shouldn't happen");
                        aggregationFxn.countFieldTerm(fields[i], fieldTermPair[1], true, aggrValue);
                    });
                });
        ...
        experiments.forEach(function(exp){
            ...
            aggregationFxn.getTermsForFieldsFromExperiment(fields,exp).forEach(function(fieldTermPair, i){
                if (fields[i].field !== fieldTermPair[0]) throw new Error("This shouldn't happen");
                aggregationFxn.countFieldTerm(fields[i], fieldTermPair[1], true, 'files', expFxn.fileCount(exp));
            });
        });
    ...
    getUniqueMatchedTermsFromExpsInSetWhereFieldIsTerm : function(experiments_in_set, field, parentField, parentTerm){
        ...
        return aggregationFxn.getUniqueMatchedTermsFromExpsInSet(
    ...
    partitionFields : function(fields, experiments, aggregate, useOnlyPopulatedFields = false){
        ...
        if (useOnlyPopulatedFields){
            ...
            nextIndex = aggregationFxn.firstPopulatedFieldIndex(fields, topIndex + 1);
        } else {
        ...
        return aggregationFxn.combinedFieldTermsForExperiments(fields, experiments, aggregate);
    },
    ...
    countFieldTerm : function(fieldObj, term, updateTotal = true, aggregateType = 'experiments', countIncrease = 1){
        ...
        if (typeof termsCont[term] !== 'object'){
            termsCont[term] = aggregationFxn.createZeroCountTermObj();
        }
    ...
    combinedFieldTermsForExperiments : function(fields, experiments, aggregate){
        ...
        function aggregateExpAndFilesFromExp(exp){
            ...
            // Files
            aggregationFxn.countFieldTerm(field.terms[topLevelFieldTerm], nextLevelFieldTerm, true, 'files', expFxn.fileCount(exp));
            // Experiments
