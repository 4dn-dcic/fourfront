    ...
    return (
        <BigDropdownIntroductionWrapper {...{ windowHeight, windowWidth, titleIcon, isActive }}>
            <h4 className="mt-0 mb-0">
                <a href={'/' + pathName} className="link-underline link-underline-opacity-0" data-handle-click={true}>{ display_title }</a>
            </h4>
                ...
                { level1ChildrenWithoutSubChildren.map(function(child){
                    ...
                    return (
                        <div className={"level-1-title-container" + (active? " active" : "")} key={child.name}>
                            <a className="link-underline link-underline-opacity-0 text-medium level-1-title" href={'/' + child.name} data-tip={child.description}
        ...
        return (
            <div className={outerCls} key={childLevel1.name}>
                <div className={"level-1-title-container" + (active ? " active" : "")}>
                    <a className="link-underline link-underline-opacity-0 text-medium level-1-title" href={'/' + childLevel1.name} data-tip={childLevel1.description}
                ...
                { hasChildren ?
                    level1Children.map(function(childLevel2){
                        return (
                            <a className={"link-underline link-underline-opacity-0 text-small level-2-title" + (urlParts.pathname.indexOf(childLevel2.name) > -1 ? ' active' : '')}
