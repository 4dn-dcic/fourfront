...
describe('Browse Views - BarPlotChart & QuickInfoBar I', function () {
    ...
    context('Filtering using visualization elements', function(){
        ...
        before(()=>{
            cy.visit('/browse/?experimentset_type=replicate&type=ExperimentSetReplicate&limit=30')
        ...
        it('Hover over & click "ChIP-seq, human" bar part + popover button --> matching filtered /browse/ results', function(){

            cy.window().scrollTo(0, 0).end()
                // A likely-to-be-here Bar Section - Repli-seq x human
                .get('.bar-plot-chart .chart-bar[data-term="ChIP-seq"] .bar-part[data-term="human"]').then(($barPart)=>{
                    const expectedFilteredResults = parseInt($barPart.attr('data-count'));
                    expect(expectedFilteredResults).to.be.greaterThan(15);
                    ...
                    return cy.window().scrollTo('top').wait(200).end()
                        ...
                        .get('.cursor-component-root .detail-crumbs .crumb').should('contain', 'ChIP-seq').end()
                        .get('.cursor-component-root .details-title .primary-count').should('contain', expectedFilteredResults).end().getQuickInfoBarCounts().then(function(origCount){
                            ...
                            return cy.window().then((w)=>{ w.scrollTo(0,0); }).end().wrap($barPart, { force: true }).scrollToCenterElement().wait(200).trigger('mouseover', { force: true }).trigger('mousemove', { force: true }).wait(300).click({ force : true }).wait(200).end()
                                ...
                                .location('search')
                                .should('include', 'experiments_in_set.experiment_type.display_title=ChIP-seq')
                ...
                cy.get('#stats .any-filters.glance-label').hoverIn().wait(100).end()
                    ...
                    .get('.bar-plot-chart .chart-bar .bar-part').should('have.length.greaterThan', 1).then(($allBarParts)=>{
                        ...
                        cy.getQuickInfoBarCounts().its('experiment_sets').should('be.greaterThan', origCounts.experiment_sets).then((unfilteredOnceExpSetCount)=>{
                            cy.get('#stats .bottom-side .field-group .chart-crumb[data-term="ChIP-seq"] i.icon-times').should('have.length', 1).click().wait(10).end()
                                .get('#stats .bottom-side .field-group .chart-crumb[data-term="ChIP-seq"] i.icon-times').should('have.length', 0).end()
                                ...
                                .should('not.include', 'experiments_in_set.experiment_type.display_title=ChIP-seq').should('not.include', 'experiments_in_set.biosample.biosource.individual.organism.name=human');
                        });
                    });
            });
